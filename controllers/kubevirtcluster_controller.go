...
func (r *KubevirtClusterReconciler) reconcileNormal(ctx *context.ClusterContext, externalLoadBalancer *loadbalancer.LoadBalancer) (ctrl.Result, error) {
	...
	if !externalLoadBalancer.IsFound() {
		if err := externalLoadBalancer.Create(ctx); err != nil {
			conditions.MarkFalse(ctx.KubevirtCluster, infrav1.LoadBalancerAvailableCondition, infrav1.LoadBalancerProvisioningFailedReason, clusterv1.ConditionSeverityWarning, "ExternalLoadBalancer service not found")
	...
	if ctx.KubevirtCluster.Spec.ControlPlaneEndpoint.Host != "" {
		...
		if err != nil {
			conditions.MarkFalse(ctx.KubevirtCluster, infrav1.LoadBalancerAvailableCondition, infrav1.LoadBalancerProvisioningFailedReason, clusterv1.ConditionSeverityWarning, "ExternalIP not found")
		...
		if err != nil {
			conditions.MarkFalse(ctx.KubevirtCluster, infrav1.LoadBalancerAvailableCondition, infrav1.LoadBalancerProvisioningFailedReason, clusterv1.ConditionSeverityWarning, "ClusterIP not found")
